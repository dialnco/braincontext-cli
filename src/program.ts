...
import { accessCommand, whoamiCommand } from './commands/access'
...
/**
 * Build the full CLI. Kept apart from the entry point (cli.ts) so the command tree
 * can be inspected without running it — `test/command-caps.test.ts` walks it to
 * prove every command declares an access capability.
 */
